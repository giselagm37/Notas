...
def login():
    if request.method == 'POST':
        ...
        if usuario == "9999" and contraseña == "9999":
            ...
            session["usuario_id"] = usuario  # <-- Esto es lo que faltaba
            session["rol"] = "admin"
        ...
        if user:
            ...
            if check_password_hash(user[3], contraseña):
                ...
                if user[2] == "admin":
                    return redirect(url_for('main.admin_dashboard'))  # Redirige al panel de administración
                else:
                    return redirect(url_for('notes.cargar_nota'))  # Redirige a la página de carga de notas
            else:
